    ...
    return (
        <div className="p-8 bg-[#2F2F2F] min-h-screen text-white">
            ...
            <form onSubmit={handleUpdate} className="bg-[#383838] p-8 rounded-lg shadow-lg max-w-5xl mx-auto">
                ...
                {/* ? Image Uploader */}
                <div className="mb-8">
                    <button type="button" onClick={() => setShowImageUploader(true)} className="w-full p-4 border-2 border-dashed text-gray-300 rounded-md hover:bg-[#444]">
                ...
                <div className="grid grid-cols-2 gap-6 mb-8">
                    <Select options={dishes} isSearchable placeholder="Select a MealKit..." onChange={handleMealKitChange} className="text-black" />
                    <input type="text" name="name" value={recipeForm.name} onChange={handleChange} placeholder="Recipe Title" className="p-3 border border-gray-500 rounded bg-[#444] text-white" required />
                    <textarea name="description" value={recipeForm.description} onChange={handleChange} placeholder="Overview of Recipe" className="p-3 border border-gray-500 rounded bg-[#444] text-white" rows="4" required />
                    <input type="number" name="timeTaken" value={recipeForm.timeTaken} onChange={handleChange} placeholder="Time Taken (mins)" className="p-3 border border-gray-500 rounded bg-[#444] text-white" />
                </div>
                ...
                {recipeForm.ingredients.map((ingredient, index) => (
                    <div key={index} className="flex gap-4 items-center mb-4">
                        <input type="text" placeholder="Quantity" value={ingredient.quantity} onChange={(e) => handleIngredientChange(index, "quantity", e.target.value)} className="p-3 border border-gray-500 rounded bg-[#444] text-white" />
                        <input type="text" placeholder="Unit" value={ingredient.unit} onChange={(e) => handleIngredientChange(index, "unit", e.target.value)} className="p-3 border border-gray-500 rounded bg-[#444] text-white" />
                        <input type="text" placeholder="Ingredient" value={ingredient.name} onChange={(e) => handleIngredientChange(index, "name", e.target.value)} className="p-3 border border-gray-500 rounded bg-[#444] text-white" />
                        <button type="button" onClick={() => handleRemoveIngredient(index)} className="text-red-400 font-bold">X</button>
                    </div>
                ))}
                <button type="button" onClick={handleAddIngredient} className="text-blue-400 font-semibold mb-6">+ Add Ingredient</button>

                {/* ? Instructions Section */}
                <h3 className="text-xl font-semibold mb-4">Instructions</h3>
                {recipeForm.instructions.map((instruction, index) => (
                    <div key={index} className="flex gap-4 items-center mb-4">
                        <textarea placeholder="Instruction Step" value={instruction.step} onChange={(e) => handleInstructionChange(index, "step", e.target.value)} className="p-3 border border-gray-500 rounded bg-[#444] text-white" rows="2"></textarea>
                        <input type="file" accept="image/*" onChange={(e) => handleInstructionImageUpload(index, e.target.files[0])} className="p-3 border border-gray-500 rounded bg-[#444] text-white" />
                        <button type="button" onClick={() => handleRemoveInstruction(index)} className="text-red-400 font-bold">X</button>
                    </div>
                ))}
                <button type="button" onClick={handleAddInstruction} className="text-blue-400 font-semibold mb-6">+ Add Instruction</button>

                {/* ? Tags, Visibility, and Submit */}
                <div className="flex justify-between items-center mt-8">
                    <input type="text" placeholder="Tags" value={recipeForm.tags} onChange={(e) => setRecipeForm({ ...recipeForm, tags: e.target.value })} className="p-3 border border-gray-500 rounded bg-[#444] text-white" required />
                    <select value={recipeForm.visibility} onChange={(e) => setRecipeForm({ ...recipeForm, visibility: e.target.value })} className="p-3 border border-gray-500 rounded bg-[#444] text-white">
                        <option value="public">Public</option>
                        <option value="private">Private</option>
                        <option value="friends-only">Friends Only</option>
                    </select>
                    <button type="submit" className="px-6 py-3 bg-blue-500 text-white font-bold rounded hover:bg-blue-700">Update Recipe</button>
                </div>
            </form>
